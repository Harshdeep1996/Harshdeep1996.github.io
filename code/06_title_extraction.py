...
def main(input_file, output_file):
    ...
    data = data.drop(columns=['title_opera_method_1', 'title_vec_method_1',
                              'predicted_title_cluster_method_1'])
    print('step 2 done')
    ...
    data['title_mediawiki_pageid'] = [ses.get(url=url, params={"action": "query",
                                       ...
                                       if ('query' in ses.get(url=url,
                                                              params={"action":
                                                                      ...
                                                                      "search",
                                                                      "srsearch": title})
                                      .json().keys()) and (len(ses.get(url=url,
                                                                       params={"action":
                                                                               "query",
                                                                       "format": "json",
                                                                       "list": "search",
    ...
    data['composer_mediawiki_pageid'] = [ses.get(url=url, params={"action": "query",
                                      ...
                                      .json().keys()) and (len(ses.get(url=url, params={"action": "query",
                                           ...
                                           'Not found'
                                       for composer in data.inferred_composer
                                       ]
...
if __name__ == "__main__":
    input_file = '../data/librettos_4.csv'
    output_file = '../data/librettos_5.csv'
