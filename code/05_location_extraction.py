...
def main(input_file, output_file):
    ...
    data['location_full_method_2'] = ['Not found' if loc == '' else loc for loc in data.location_full_method_2]
    data['location_vec_method_1'] = [np.mean(np.array([token.vector for token in nlp(loc)]), axis=0)
                                     for loc in data.location_full_method_2]
    ...
    for i, cluster in enumerate(data.predicted_cluster_method_1):
        if data.location_latitude_method_2[i] != '':
            if cluster in cluster_theater_latitude.keys():
                cluster_theater_latitude[cluster].append(data.location_latitude_method_2[i])
                cluster_theater_longitude[cluster].append(data.location_longitude_method_2[i])
                cluster_theater[cluster].append(data.location_full_method_2[i])
            else:
                cluster_theater_latitude[cluster] = [data.location_latitude_method_2[i]]
                cluster_theater_longitude[cluster] = [data.location_longitude_method_2[i]]
                cluster_theater[cluster] = [data.location_full_method_2[i]]
    ...
    data = data.drop(columns=['location_method_1', 'location_type_method_1',
...
if __name__ == "__main__":
    input_file = '../data/librettos_3.csv'
    output_file = '../data/librettos_4.csv'
